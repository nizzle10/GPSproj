...
#GPXd<-readGPX(args)
#GPXd<-readGPX('1_02_2021.gpx')

HSRDfunc<-function(gpsD){
   ...
   mn <- 5.5
   mx <- max(STD$Speed)
   STD$SpeedBracket <- ifelse(STD$Speed>mn&STD$Speed<mx,TRUE,FALSE)
   HSRDresult<-sum(STD$Distances[STD$SpeedBracket==TRUE])
   #date <- STD$Times[1]
   return(HSRDresult)
}
...
#    STD$accel[i+1] <- (v-u)/t 
# }
# STD$accel[length(STD$accel)]<-0
# mn <- 3
# mx <- max(STD$accel)
# STD$AccelBracket <- ifelse(STD$accel>mn&STD$accel<mx,TRUE,FALSE)
# mn <- -3
# mx <- min(STD$accel)
# 
# STD$DeccelBracket <- ifelse(STD$accel>mn&STD$accel<mx,TRUE,FALSE)
# ar <- STD$accel[STD$DeccelBracket==TRUE]
# dr <- STD$accel[STD$DeccelBracket==TRUE]
